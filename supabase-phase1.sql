-- ============================================
-- LegalAI Phase 1: Firms, Contracts, Storage
-- Run AFTER supabase-auth-setup.sql
-- Multi-tenant: all data scoped by firm_id
-- ============================================
...
-- --------------------------------------------
-- 2. ADD FIRM_ID TO PROFILES (must be before get_my_firm_id)
-- --------------------------------------------
...
-- --------------------------------------------
-- 4. UPDATE SIGNUP TRIGGER: create firm + profile
-- --------------------------------------------
...
BEGIN
  ...
  IF user_name = '' THEN user_name := 'My Practice'; END IF;
...
-- --------------------------------------------
-- 5. BACKFILL: create firms for existing profiles without firm_id
-- --------------------------------------------
DO $$
DECLARE
  prof RECORD;
...
BEGIN
  ...
  LOOP
    INSERT INTO public.firms (name)
    VALUES (COALESCE(NULLIF(trim(prof.name), ''), split_part(prof.email, '@', 1), 'My Practice') || '''s Practice')
    RETURNING id INTO new_firm_id;
...
-- 7. STORAGE BUCKET
-- --------------------------------------------
-- Create bucket if it doesn't exist. If this fails, create manually: Dashboard → Storage → New bucket → id: contracts
DO $$
BEGIN
...
-- Storage policies: path = firm_id/contract_id/filename
